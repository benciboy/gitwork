function [H, pValue, KSstatistic] = kstest2_bin(x1, x2, binSize, nBins, alpha, tail)
%KSTEST2_BIN Two-sample Kolmogorov-Smirnov goodness-of-fit hypothesis test.
...
%
% removing bigger elements like binSize*nBins
x1(x1>(binSize*nBins))=[];
x2(x2>(binSize*nBins))=[];

%adding limits 
x1 = [0 ; x1 ; binSize*nBins];
x2 = [0 ; x2 ; binSize*nBins];

%binEdges    =  [0 ; sort([x1;x2]) ; binSize*nBins];

binCounts1  =  hist (x1 , nBins);
binCounts2  =  hist (x2 , nBins);
